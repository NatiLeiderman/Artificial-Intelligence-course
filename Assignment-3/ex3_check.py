...
def solve(game: ext_plant.Game, run_idx: int, controller_module, verbose: bool = True):
    ...
    return {
        ...
        "steps": game.get_max_steps(),
    }


def _run_single_seed(problem: dict, seed: int, debug_mode: bool, controller_name: str):
    local_problem = copy.deepcopy(problem)
    local_problem["seed"] = seed
    game = ext_plant.create_pressure_plate_game((local_problem, debug_mode))
    controller_module = ex3_random if controller_name == "ex3_random" else ex3
    run_start = time.time()
    result = solve(game, seed, controller_module, verbose=False)
...
def main():
    ...
    for idx, (pname, problem) in enumerate(problems, start=1):
        ...
        max_workers = min(os.cpu_count() or 1, n_runs)
        if max_workers <= 1:
            for seed in range(n_runs):
                try:
                    result = _run_single_seed(
                        problem, seed, debug_mode, controller_name
                    )
            ...
            with concurrent.futures.ProcessPoolExecutor(
                ...
                futures = {
                    executor.submit(
                        _run_single_seed, problem, seed, debug_mode, controller_name
                    ): seed
                    for seed in range(n_runs)
                }
        ...
        for seed in range(n_runs):
            ...
            run_reward = result["reward"]
            run_duration = result["duration"]
            state_str = result["state_str"]
            finished_txt = result["finished_txt"]
            color = result["finished_color"]
            steps = result["steps"]

            print(
                f"Run {seed:2d}: {BOLD}{YELLOW}Reward: {run_reward:3d}{RESET} | Steps: {steps:2d} | {color}{finished_txt}{RESET} \nState: {state_str}"
            )
